...
def makescript(input=None,indir=None,name=None,scriptname=None,idle=False,prefix=None,hyperthread=True)
    ...
    indir     The directory to put the job script in.
    ...
    USAGE:
    job_makescript,input,dir=dir,name=name,idle=idle,scriptname=scriptname,$
    ...
    # Not enough inputs
    if (input is None):
        raise ValueError('Syntax - job_makescript,input,dir=dir,name=name,scriptname=scriptname,idle=idle,prefix=prefix,hyperthread=hyperthread')

    ninput = len(np.array(input))
    if indir is not None: ndir=len(np.array(indir)) else ndir=0
    if name is not None: nname=len(np.array(name)) else nname=0
    ...
    if nname==0:
        ...
        if prefix is not None: pre=prefix[0] else pre='pr'
...
def checkstat(statstr=None,jobid=None,hyperthread=True):
    ...
    if jobid is None: njob=0 else njob=len(np.array(jobid))
