...
def trf(fun, jac, x0, f0, J0, lb, ub, ftol, xtol, gtol, max_nfev, x_scale,
        loss_function, tr_solver, tr_options, verbose, dx_lim=None):
    ...
    if np.all(lb == -np.inf) and np.all(ub == np.inf):
        return trf_no_bounds(
            ...
            loss_function, tr_solver, tr_options, verbose, dx_lim=dx_lim)
    else:
        return trf_bounds(
...
def trf_bounds(fun, jac, x0, f0, J0, lb, ub, ftol, xtol, gtol, max_nfev,
               x_scale, loss_function, tr_solver, tr_options, verbose, dx_lim=None):
    x = x0.copy()
